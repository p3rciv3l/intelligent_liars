...
def _validate_prompt_inventory(prompts: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    for record_id, row in prompts.items():
        ...
        if expected not in OBSERVED_BEHAVIORS:
            raise ValueError(
                f"unsupported expected_behavior for {record_id}: {expected!r}"
            )
...
def build_response_inventory(
    ...
    *,
    ...
    run_identities = {
        str(row.get("run_identity_sha256", "")) for row in responses.values()
    }
    if len(run_identities) != 1:
        raise ValueError("responses must identify exactly one inference run")
    run_identity = next(iter(run_identities))
    _require_sha256(run_identity, field="run_identity_sha256")
    ...
    *,
    ...
    for record_id, row in inventory.items():
        ...
        if row.get("source_plan_sha256") != source_plan_sha256:
            raise ValueError(
                f"source plan hash mismatch in response inventory: {record_id}"
            )
        expected = str(row.get("expected_behavior", ""))
        if expected not in OBSERVED_BEHAVIORS:
            raise ValueError(
        ...
        run_identities.add(str(row.get("run_identity_sha256", "")))
    ...
    if len(run_identities) != 1:
        raise ValueError("response inventory must identify exactly one inference run")
    _require_sha256(next(iter(run_identities)), field="run_identity_sha256")
    ...
    *,
    ...
    for record_id in sorted(inventory):
        ...
        scored_records.append(
            {
                "record_id": record_id,
                "split_group_id": str(inventory[record_id]["split_group_id"]),
                "xstest_type": str(inventory[record_id]["xstest_type"]),
    ...
    return {
        ...
        "model_identity": next(
            iter({row["model_identity"] for row in inventory.values()})
        ),
        "run_identity_sha256": next(
            iter({row["run_identity_sha256"] for row in inventory.values()})
        ),
        **metrics,
        "by_xstest_type": {
