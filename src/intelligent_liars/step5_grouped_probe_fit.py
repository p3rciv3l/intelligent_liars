...
def derive_legacy_example_identity(
    *,
    ...
    example_group_id = f"legacy/{task_id}/source:{source_index}"
    example_id = f"{example_group_id}/output:{output_index}"
    ...
    return LegacyExampleIdentity(
        ...
        source_group_id=task_id,
        example_group_id=example_group_id,
        template_group_id=f"legacy/{task_id}/{template_suffix}",
    )
...
def build_outer_split(
    ...
    *,
    evaluator_source_group_ids: set[str],
) -> OuterSplit:
    available = {identity.source_group_id for identity in identities}
    missing = sorted(evaluator_source_group_ids - available)
    if missing:
        raise ValueError(f"Configured evaluator source groups are absent: {missing}")
    regularizer = np.asarray(
        [i for i, identity in enumerate(identities) if identity.source_group_id not in evaluator_source_group_ids],
        dtype=np.int64,
    )
    evaluator = np.asarray(
        [i for i, identity in enumerate(identities) if identity.source_group_id in evaluator_source_group_ids],
        dtype=np.int64,
    )
    ...
    return OuterSplit(
        ...
        regularizer_source_groups=tuple(sorted(available - evaluator_source_group_ids)),
        evaluator_source_groups=tuple(sorted(evaluator_source_group_ids)),
    )
...
class CrossfitFold:
    fold: int
    heldout_source_group_id: str
    train_indices: np.ndarray
    test_indices: np.ndarray


def build_crossfit_plan(
    identities: Sequence[LegacyExampleIdentity], indices: np.ndarray
) -> tuple[CrossfitFold, ...]:
    groups = sorted({identities[int(i)].source_group_id for i in indices})
    if len(groups) < 2:
        raise ValueError("Grouped cross-fit requires at least two source groups")
    folds = []
    for fold, heldout in enumerate(groups):
        test = np.asarray(
            [int(i) for i in indices if identities[int(i)].source_group_id == heldout],
            dtype=np.int64,
        )
        train = np.asarray(
            [int(i) for i in indices if identities[int(i)].source_group_id != heldout],
...
def task_balanced_indices(
    ...
    *,
    ...
    selected: list[int] = []
    groups = sorted({identities[int(i)].source_group_id for i in eligible_indices})
    for group in groups:
        for label in (0, 1):
            candidates = np.asarray(
                [
                    ...
                    if identities[int(i)].source_group_id == group and int(labels[int(i)]) == label
                ],
                dtype=np.int64,
            )
...
def _macro_task_auc(
    ...
    values = []
    for task in sorted({identities[int(i)].source_group_id for i in indices}):
        mask = np.asarray([identities[int(i)].source_group_id == task for i in indices])
...
def _raw_metadata_by_task(raw_activation_cache: Path, tasks: Sequence[str]) -> dict[str, list[str | bytes | None]]:
    import h5py

    result: dict[str, list[str | bytes | None]] = {}
    with h5py.File(raw_activation_cache, "r") as handle:
        ...
        for task in tasks:
            if metadata is None or task not in metadata or "example_metadata_json" not in metadata[task]:
                ...
                result[task] = list(metadata[task]["example_metadata_json"][:])
    return result
...
def load_legacy_identity_and_labels(
    ...
    with h5py.File(pooled_cache, "r") as handle:
        ...
        raw_metadata = _raw_metadata_by_task(raw_activation_cache, tasks)
        ...
        for task in tasks:
            ...
            metadata_rows = raw_metadata.get(task, [])
    ...
    with h5py.File(pooled_cache, "r") as handle:
        ...
        for task, global_indices in task_local_to_global.items():
            if int(layer_group[task].shape[0]) != len(global_indices):
                raise ValueError(f"Feature row count differs for {task} at layer {layer}")
            local_rows = [
                row
            ...
            if not local_rows:
                continue
            values = np.asarray(layer_group[task][local_rows], dtype=np.float32)
...
def run_grouped_probe_fit(
    *,
    ...
    if pooling_candidates != [provenance["pooling"]]:
        raise ValueError("Pooling calibration candidates must exactly match available cache pooling")
    outer = build_outer_split(
        identities,
        evaluator_source_group_ids={_identifier_component(value) for value in config["evaluator_source_group_ids"]},
    )
    ...
    for layer in calibration["candidate_layers"]:
        ...
        if not fold_aucs:
            raise ValueError(f"No valid calibration folds for layer {layer}")
        calibration_rows.append(
            {"layer": layer, "token_pooling": provenance["pooling"], "fold_macro_task_roc_auc": fold_aucs, "mean_macro_task_roc_auc": float(np.mean(fold_aucs))}
        )
    ...
    registry_probes = [
        {
            ...
            "layer": selected_layer,
            "token_pooling": provenance["pooling"],
            "direction_sign_convention": sign,
        }
    ]
    crossfit_rows = []
    for fold in build_crossfit_plan(identities, evaluator_sample):
        ...
        _atomic_json(
            ...
            _probe_payload(
                ...
                evaluation={"kind": "leave_one_source_group_out", "heldout_source_group_id": fold.heldout_source_group_id, "heldout_example_count": int(len(fold.test_indices)), "heldout_metrics": heldout_metrics},
                identities=identities,
                step5_plan_manifest_sha256=step5_plan_sha256,
            ),
        )
        crossfit_rows.append({"probe_id": probe_id, "heldout_source_group_id": fold.heldout_source_group_id, **heldout_metrics})
