...
PINNED_README_SHA256 = "1422c370346c2d6a1f382db769c6d600995ceead1c064f88220f9f9e7bb39cb6"
...
def build_snapshot_plan(hub_model_info: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "excluded_repository_files": sorted(set(by_path) - set(REQUIRED_SNAPSHOT_FILES)),
    }
...
def _validate_plan(plan: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    ...
    if plan.get("model") != {"repo_id": MODEL_ID, "revision": MODEL_REVISION}:
        raise CacheValidationError("snapshot plan does not pin the approved model revision")
    if plan.get("allow_patterns") != list(REQUIRED_SNAPSHOT_FILES):
        raise CacheValidationError("snapshot plan does not use the exact narrow inventory")
...
def verify_snapshot(
    ...
    if not snapshot_root.is_dir():
        raise CacheValidationError(f"snapshot directory does not exist: {snapshot_root}")
    ...
    for item in planned_files:
        ...
        if path.is_symlink() or not path.is_file():
            raise CacheValidationError(f"not a self-contained regular file: {item['path']}")
...
def materialize_huggingface_cache(
    ...
    for source_entry, verified_entry in zip(planned, verified, strict=True):
        ...
        if blob_path.exists():
            if not blob_path.is_file() or sha256_file(blob_path) != verified_entry["sha256"]:
                raise CacheValidationError(f"corrupt existing Hugging Face blob: {blob_path}")
        else:
...
def completion_marker(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected_suffix = (
        ...
        f"{MODEL_REVISION}/{manifest['content_sha256']}"
    )
    object_prefix = s3.get("object_prefix")
    if not isinstance(object_prefix, str) or not (
        f"/{object_prefix}".endswith(expected_suffix)
    ):
        raise CacheValidationError("cache manifest has an inconsistent S3 object prefix")
    expected_s3 = {
        "schema": "tinylora_model_cache_s3_v1",
