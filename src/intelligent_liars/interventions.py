...
def writer_edit_coefficient(spec: InterventionSpec) -> float:
    if spec.method == InterventionMethod.AFFINE_PROJECTION:
        if spec.projection_target != 0.0:
            raise ValueError(
                "A non-zero affine projection target cannot be baked into bias-free writers"
            )
        return 1.0
    if spec.method == InterventionMethod.FULL_REFLECTION:
        return 2.0
    if spec.method == InterventionMethod.PARTIAL_REFLECTION:
        return 2.0 * spec.reflection_strength
    raise ValueError(
        f"{spec.method.value} is state-dependent or affine and requires a runtime intervention"
    )


def validate_materializable_bundle(bundle: InterventionBundle) -> None:
    validate_probe_direction(bundle.direction)
    validate_intervention_spec(bundle.spec)
    ...
    if direction.intercept != 0.0:
        raise ValueError(
            "A non-zero probe intercept requires a runtime intervention; a bias-free "
            "writer edit can only transform a direction through the origin"
        )
    if bundle.spec.token_scope != TokenScope.ALL:
        raise ValueError("A persistent writer edit requires token_scope='all'")
    if bundle.spec.max_score_delta is not None:
        raise ValueError(
            "A bounded score adjustment cannot be materialized as a writer edit"
        )


def apply_rank_one_writer_edit(model: Any, bundle: InterventionBundle) -> list[str]:
    validate_materializable_bundle(bundle)
    coefficient = writer_edit_coefficient(bundle.spec)
    direction = bundle.effective_direction().tensor()
    direction = direction / torch.linalg.vector_norm(direction)
    layers = qwen_language_layers(model)
    invalid_layers = [layer for layer in bundle.spec.layers if layer >= len(layers)]
    if invalid_layers:
        raise ValueError(
            f"Intervention layers {invalid_layers} are outside model with {len(layers)} layers"
        )
    selected_writers: list[tuple[str, Any]] = []
    for layer_idx in bundle.spec.layers:
        layer = layers[layer_idx]
        selected_writers.extend(
            [
                (
    ...
    for name, module in selected_writers:
        if module.weight.shape[0] != direction.numel():
            raise ValueError(
                f"Writer {name} output dimension {module.weight.shape[0]} does not match "
                f"direction dimension {direction.numel()}"
            )

    edited: list[str] = []
    for name, module in selected_writers:
        weight = module.weight
        unit = direction.to(device=weight.device, dtype=weight.dtype)
        with torch.no_grad():
            projection = unit.unsqueeze(1) @ (unit.unsqueeze(0) @ weight)
            weight.sub_(coefficient * projection)
        edited.append(name)
    return edited


def save_writer_edited_model(
    *,
    ...
    if model_bundle.model is None:
        raise ValueError("Model bundle does not contain model weights")
    if output_dir.exists():
        raise FileExistsError(f"Output model directory already exists: {output_dir}")
    edited = apply_rank_one_writer_edit(model_bundle.model, bundle)
    output_dir.mkdir(parents=True)
    model_bundle.model.save_pretrained(output_dir, safe_serialization=True)
    model_bundle.processor.save_pretrained(output_dir)
    save_intervention_bundle(bundle, output_dir / "intervention.json")
    (output_dir / "writer_edit_materialization.json").write_text(
        json.dumps(
            {
                "format": "qwen_truth_writer_edit_v1",
                "implementation": "rank_one_writer_edit",
                "writer_coefficient": writer_edit_coefficient(bundle.spec),
                "edited_parameters": edited,
                "caveat": (
                    "Writer-space reflection/attenuation is a persistent rank-one model edit. "
                    "Zero-target projection is equivalent to global directional ablation only "
                    "when every residual-stream writer is edited. This checkpoint edits only "
                    "the selected decoder writers, and other coefficients are not equivalent "
                    "to a single-layer runtime transform."
                ),
            },
            indent=2,
            sort_keys=True,
