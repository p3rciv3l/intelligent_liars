...
def test_topk_preservation_kl_bounds_full_vocabulary_reductions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    student = torch.zeros((1, 34, 131_072), dtype=torch.float16)
    indices = torch.arange(64).reshape(1, 1, 64).expand(1, 34, 64)
    probabilities = torch.full((1, 34, 65), 1.0 / 65.0)
    mask = torch.ones((1, 34), dtype=torch.bool)
    real_logsumexp = torch.logsumexp
    largest_call = 0

    def bounded_logsumexp(
        value: torch.Tensor, dim: int, keepdim: bool = False
    ) -> torch.Tensor:
        nonlocal largest_call
        largest_call = max(largest_call, value.numel())
        if value.numel() > 4_194_304:
            raise torch.OutOfMemoryError("synthetic full-vocabulary reduction OOM")
        return real_logsumexp(value, dim=dim, keepdim=keepdim)

    monkeypatch.setattr(torch, "logsumexp", bounded_logsumexp)
...
def test_topk_preservation_kl_stays_finite_for_extreme_finite_bfloat16_logits():
