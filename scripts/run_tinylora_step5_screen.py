...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        choices=("reachability", "smoke", "prerequisites", "train"),
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Frozen receipt generated by --mode prerequisites; required in train mode.",
    )
    parser.add_argument(
        "--overfit-min-relative-loss-reduction",
        type=float,
        default=0.20,
        help="Frozen minimum relative loss reduction for prerequisite generation.",
    )
...
def validate_numeric_args(args: argparse.Namespace) -> None:
    ...
    reduction = float(args.overfit_min_relative_loss_reduction)
    if reduction != MIN_INTENTIONAL_OVERFIT_RELATIVE_LOSS_REDUCTION:
        raise ValueError("intentional-overfit threshold is frozen at 0.20")
    if args.mode == "prerequisites" and args.max_steps < 20:
        raise ValueError("prerequisites mode requires at least 20 optimizer steps")
...
def validate_durability_args(args: argparse.Namespace) -> None:
    ...
    if args.mode == "train" and args.prerequisite_receipt is None:
        raise ValueError("train mode requires --prerequisite-receipt")
    ...
    if args.mode == "prerequisites" and (
        not isinstance(image_digest, str)
...
def run_reachability(
    *,
    ...
    if set(means) != EXPECTED_REACHABILITY_OBJECTIVES:
        raise ValueError("Reachability did not cover every frozen Step 5 objective")
    return {
        "format": "tinylora_step5_reachability_v1",
        "selected_records": len(selected),
        "selected_record_ids": [str(row["record_id"]) for row in selected],
        "unexplained_skips": 0,
...
def _intentional_overfit_rows(
    behavior_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Choose a deterministic tiny diagnostic set spanning every behavior objective."""
    grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in behavior_rows:
        grouped[str(row["objective"])].append(row)
    selected = [
        sorted(rows, key=lambda row: str(row["record_id"]))[0]
        for _objective, rows in sorted(grouped.items())
        if rows
    ]
    if not selected:
        raise ValueError("Intentional-overfit prerequisite has no behavior rows")
    return selected


def _mean_behavior_loss(
    *,
    model: Any,
    processor: Any,
    capture: LayerCapture,
    ...
    """Measure the exact paired behavior objective on a small frozen row set."""
    device = next(model.parameters()).device
    values: list[float] = []
    model.eval()
    with torch.no_grad():
        for row in rows:
            ...
            if not math.isfinite(value) or value < 0:
                raise FloatingPointError("Intentional-overfit loss is invalid")
            values.append(value)
    return sum(values) / len(values)


def prerequisite_evaluator_metric_rows(
    evaluation: Mapping[str, Any],
) -> list[dict[str, Any]]:
    """Extract the evidence fields emitted by ``evaluate_arm`` for a receipt."""
    behavior = evaluation.get("behavior")
    if not isinstance(behavior, list):
        raise ValueError("Prerequisite evaluator output lacks behavior rows")
    return [
        {
            "record_id": row["record_id"],
            "base_margin": row["base_margin"],
            "student_margin": row["student_margin"],
            "improvement": row["improvement"],
            "direction_movement": row["direction_movement"],
        }
        for row in behavior
    ]
...
def main() -> int:
    ...
    prerequisite_contract = prerequisite_identity(
        plan_sha256=file_sha256(plan_path),
        probe_sha256=file_sha256(args.probe),
        code_sha256=code_sha256,
        arm=arm,
        model=model_specification,
        runtime_image_digest=args.runtime_image_digest or "",
    )
    prerequisite_receipt_sha256: str | None = None
    if args.mode == "train":
        assert args.prerequisite_receipt is not None
        _receipt, prerequisite_receipt_sha256 = read_and_validate_prerequisite_receipt(
            args.prerequisite_receipt.resolve(),
            expected_identity=prerequisite_contract,
        )
    ...
    if args.mode == "reachability":
        ...
        training_rows = schedule
        initial_mean_loss: float | None = None
        prerequisite_reachability: dict[str, Any] | None = None
        if args.mode == "prerequisites":
            training_rows = _intentional_overfit_rows(behavior_train)
            prerequisite_reachability = run_reachability(
                model=model,
                processor=bundle.processor,
                capture=capture,
                parameters=parameters,
            ...
            initial_mean_loss = _mean_behavior_loss(
                model=model,
                processor=bundle.processor,
                capture=capture,
                parameters=parameters,
        ...
        evaluation_behavior = (
            training_rows if args.mode == "prerequisites" else development_behavior
        )
        evaluation_preservation = (
            [] if args.mode == "prerequisites" else preservation_development
        )
        evaluation = evaluate_arm(
            ...
            capture=capture,
            parameters=parameters,
            behavior_rows=evaluation_behavior,
            preservation_rows=evaluation_preservation,
            direction=direction,
            intercept=intercept,
            max_length=args.max_length,
            seed=args.seed,
        ...
        if args.mode == "prerequisites":
            ...
            assert prerequisite_reachability is not None
            final_mean_loss = _mean_behavior_loss(
                model=model,
                processor=bundle.processor,
                capture=capture,
                parameters=parameters,
            ...
            evaluator_metric_rows = prerequisite_evaluator_metric_rows(evaluation)
            receipt = build_prerequisite_receipt(
                identity=prerequisite_contract,
                reachability=prerequisite_reachability,
                initial_mean_loss=initial_mean_loss,
                final_mean_loss=final_mean_loss,
                optimizer_steps=int(training["optimizer_steps"]),
                selected_records=len(training_rows),
                required_relative_loss_reduction=(
                    args.overfit_min_relative_loss_reduction
                ),
                evaluator_expected_record_ids=[
                    str(row["record_id"]) for row in training_rows
                ],
                evaluator_metric_rows=evaluator_metric_rows,
            )
            receipt_path = args.output_dir / "prerequisite_receipt.json"
            atomic_json(receipt_path, receipt)
            prerequisite_receipt_sha256 = file_sha256(receipt_path)
            result["prerequisite_receipt"] = receipt
    capture.close()
    result.update(
        {
            "arm": arm,
