...
The lifecycle wrapper now uses a source allowlist, measured host qualification, same-instance recovery, controller-side durable verification, an exact artifact inventory, and a maximum-three-worker lock. The narrow model cache, corrected real-image bundle, and clean frozen run inputs are durably published. Credentialless hydration, signed durable checkpoint acknowledgements, and controller-verified final artifact publication are implemented. The launch validator authenticates the trusted input presigner receipt, generated URL manifest, host-gate object, account, region, bucket, object hashes, and approval-time freshness. It separately authenticates the final artifact PUT receipt and URL against the immutable destination, create-only header, account, region, approval, and expiry; the lifecycle wrapper repeats that check before creation and every same-instance restart. On same-host resume, the hydrator verifies an already-complete exact receipt/cache before reading the protected URL, so an expired presigned URL does not prevent recovery. The runtime image is published and independently verified at registry-index digest `sha256:ba0cf29cb29b47e47f6dd2448124ab4ddf061a95811d6f5bf036b73b00678e5f` from source commit `6e4f32d724b5fa0b752e911669691136bfa37efd`. The user explicitly approved an uncapped-money mode: the exact offer and hourly price remain approval-bound, but there is no maximum-dollar or total-runtime ceiling. Controller operations remain bounded, a 1,800-second no-progress watchdog remains active, and the controller refreshes the immutable-destination artifact upload authorization so URL expiry does not become a hidden runtime cap. Refresh timing is bounded by the actual temporary AWS credential expiry: a 15-minute session refreshes by 480 seconds, and start/restart fails closed at 300 seconds remaining. After a start with more than that reserve, monitored maintenance revalidates and atomically re-stages the exact capability during the active workload; refresh failure stops the worker for recovery. The live packet now freezes offer `43979795`, the approved hourly price, AWS identity/region, Enabled bucket-versioning proof, and protected controller receipts with zero remaining substitutions. It is launch-ready but remains inert until the controller separately adds both execution confirmations. Passing one exact-image canary remains required before the three-arm screen, and the larger run remains disabled pending separate authorization.
...
- The exact Qwen model snapshot is locally cached and hash-verified at revision `92f3c4b4feadd3a016ef468d103bb5f58b2a2c6b`: 14 runtime files totaling 17,545,907,058 bytes. Its content identity is `bbca6a8b09a56f0c538887b82b9594b0c0945c5fbcde54f39eda153a9f64eda8`. Its 14 runtime files, four legal/attribution files, manifest, and completion marker are durably published as 20 objects under the compatible `model-cache/v2/.../sha256meta/` prefix. Large shards expose an S3 `FULL_OBJECT` checksum plus matching `metadata.sha256`; all objects were independently verified and the completion marker was published last.
